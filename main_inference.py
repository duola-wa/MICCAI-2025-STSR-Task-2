...
# def icp_refinement(p_src, p_tgt, initial_transform, max_iterations=50):
#     """
#     ICP精细化函数
#
#     Args:
#         p_src: numpy array [N, 3] - STL点云
#         p_tgt: numpy array [M, 3] - CBCT点云
#         initial_transform: numpy array [4, 4] - 深度学习预测的变换矩阵
#         max_iterations: ICP最大迭代次数
#
#     Returns:
#         refined_transform: numpy array [4, 4] - 精细化后的变换矩阵
#     """
#     # 1. 转换为Open3D格式
#     source_pcd = o3d.geometry.PointCloud()
#     source_pcd.points = o3d.utility.Vector3dVector(p_src)
#
#     target_pcd = o3d.geometry.PointCloud()
#     target_pcd.points = o3d.utility.Vector3dVector(p_tgt)
#
#     # 2. 使用深度学习结果作为ICP的初始变换
#     source_pcd.transform(initial_transform)
#
#     # 3. ICP配准
#     icp_result = o3d.pipelines.registration.registration_icp(
#         source_pcd, target_pcd,
#         max_correspondence_distance=10.0,  # 最大对应距离10mm
#         init=np.eye(4),  # 已经应用初始变换，这里用单位矩阵
...
#     )
#
#     # 4. 组合变换：最终变换 = ICP变换 ○ 初始变换
#     refined_transform = icp_result.transformation @ initial_transform
#
#     print(f"ICP收敛信息: fitness={icp_result.fitness:.4f}, rmse={icp_result.inlier_rmse:.4f}")
#
...
def predict_single_case(model, device, config, stl_path, cbct_path):
    ...
    coords = np.argwhere(cbct_data > 600)
...
#
#     T_initial = np.eye(4)
#     T_initial[:3, :3] = R_pred
#     T_initial[:3, 3] = t_final
#
#     # === 新增：ICP后处理 ===
#     print("  - 开始ICP精细化...")
#     T_refined = icp_refinement(p_src, p_tgt, T_initial, max_iterations=50)
#     print("  - ICP精细化完成!")
#
#     return T_refined  # 返回精细化后的结果，而不是T_initial
def main():
    print("12313")
